...
branch can be collapsed if the split point >= 10.  So when performing a binary
  ...
  if node.__class__ is Leaf:
    ...
    for split, child in zip(node.splits, node.children):
      if child.__class__ is Leaf:
        print '%s(%s < %s) -> %s' % (
            indent, split.feature, split.max, child.value)
      else:
...
def get_weighted_splits(node, weight=1.0):
  ...
  if node.__class__ is Leaf:
    ...
    for child in node.children:
      ...
      for weighted_split in get_weighted_splits(child,
...
def filter_by_split(node, split):
  ...
  if node.__class__ is Leaf:
    ...
    if node.feature == split.feature:
      ...
      return Tree(node.feature, node.splits,
                  [filter_by_split(child, split) for child in node.children])
  elif node.__class__ is Forest:
